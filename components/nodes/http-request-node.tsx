    ...
    <Card className={`min-w-[280px] max-w-[400px] border-2 bg-card transition-all ${getStatusColor(status, selected)}`}>
      <div className="flex items-center gap-3 border-b border-border px-4 py-3">
        ...
        <div className="flex-1">
          <h3 className="text-sm font-semibold text-foreground">{t("nodes.httpRequest.label")}</h3>
      ...
      <div className="space-y-2 p-4">
        <div className="text-xs">
          <span className="text-muted-foreground">{t("config.httpRequest.url")}:</span>
      ...
      {data.output && (
        <div className="border-t border-border bg-secondary/30 p-3">
          <p className="mb-1 text-xs font-medium text-muted-foreground">{t("execution.title")}:</p>

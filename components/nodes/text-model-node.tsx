    ...
    <Card className={`min-w-[280px] max-w-[400px] border-2 bg-card transition-all ${getStatusColor(status, selected)}`}>
      <div className="flex items-center gap-3 border-b border-border px-4 py-3">
        ...
        <div className="flex-1">
          <h3 className="text-sm font-semibold text-foreground">{t("nodes.textModel.label")}</h3>
      ...
      <div className="space-y-2 p-4">
        <div className="flex justify-between text-xs">
          <span className="text-muted-foreground">{t("nodes.textModel.temperature")}</span>
          <span className="font-mono text-foreground">{data.temperature || 0.7}</span>
        </div>
        <div className="flex justify-between text-xs">
          <span className="text-muted-foreground">{t("nodes.textModel.maxTokens")}</span>
          <span className="font-mono text-foreground">{data.maxTokens || 2000}</span>
        </div>
        {data.structuredOutput && (
          <div className="flex justify-between text-xs">
            <span className="text-muted-foreground">{t("nodes.textModel.structured")}</span>

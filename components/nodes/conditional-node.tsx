    ...
    <Card className={`min-w-[280px] max-w-[400px] border-2 bg-card transition-all ${getStatusColor(status, selected)}`}>
      <div className="flex items-center gap-3 border-b border-border px-4 py-3">
        ...
        <div className="flex-1">
          <h3 className="text-sm font-semibold text-foreground">{t("nodes.conditional.label")}</h3>
          <p className="text-xs text-muted-foreground">{t("nodes.conditional.description")}</p>
        </div>
      </div>

      <div className="space-y-2 p-4">
        <div className="text-xs">
          <span className="text-muted-foreground">{t("nodes.conditional.condition")}:</span>
        ...
        {status === "running" && (
          <div className="flex items-center gap-2 text-xs text-yellow-600">
            ...
            {t("nodes.conditional.evaluating")}...
          </div>
        )}
      </div>
      ...
      {data.output !== undefined && (
        <div className="border-t border-border bg-secondary/30 p-3">
          <p className="mb-1 text-xs font-medium text-muted-foreground">{t("nodes.conditional.result")}:</p>
